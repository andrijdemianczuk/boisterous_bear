...
def ad_dlt_dts2_parsed():
  ...
  read_stream = dlt.readStream("ad_dlt_dts2_raw") \
...
def ad_dlt_dts2_seq():
  ...
  df = dlt.readStream("ad_dlt_dts2_parsed") \
  .select("*")
...
@dlt.table(comment="The first formatted table to be used for the surface map render. Handles column transformation", table_properties={"pipelines.reset.allowed": "true"})
def ad_dlt_dts2_melted():
  
  df = dlt.readStream("ad_dlt_dts2_parsed").select("*").withColumn("timestamp", col("timestamp").cast("TimeStamp"))
