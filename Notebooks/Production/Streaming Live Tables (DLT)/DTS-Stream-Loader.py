...
def ad_dlt_dts_parsed():
  
  #Define the schema from the sample file
  sampleDF = spark.read.format("json").load(f"dbfs:/FileStore/Users/andrij.demianczuk@databricks.com/resources/dts_test1.json")
  schema = sampleDF.schema.json()
  
  #Read the stream from the upstream live table
  read_stream = dlt.readStream("ad_dlt_dts_raw") \
  .select(col("key").cast("string").alias("eventId"), from_json(col("value").cast("string"), schema).alias("json")) \
  .select("json.*")
...
@dlt.table(comment="The first formatted table to be used for the surface map render. Handles column transformation", table_properties={"pipelines.reset.allowed": "true"})
def ad_dlt_dts_melted():
  
  df = dlt.readStream("ad_dlt_dts_parsed").select("*").withColumn("timestamp", col("timestamp").cast("TimeStamp"))
  
  #Still trying to understand if melt() or unpivot() is better. Will need to benchmark later.
